...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++) // number of ranks = number of candidates
        {
            int choice = preferences[i][j];
            if (candidates[choice].eliminated == false)
            {
                candidates[choice].votes++;
                break; // stop rank loop, go to next voter
            }
            // if candidate eliminated, go to next rank
        }
...
bool print_winner(void)
{
    int votes_to_win = ceil(voter_count * 0.5);
    for (int i = 0; i < candidate_count; i++)
    {
