...
-- check for collision with any of four flag/pole blocks
function Player:checkFlagCollision()
    if self.dx > 0 then
        -- check right top and bottom corners
        if self.map:isFlag(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:isFlag(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
...
function Player:checkLeftCollision()
    if self.dx < 0 then
        -- check if there's a tile directly left to us
        -- check left top and bottom corners
...
function Player:checkRightCollision()
    if self.dx > 0 then
        -- check if there's a tile directly right to us
        -- check right top and bottom corners
